...
type BadColor = "#43b581" | "#faa61a" | string & {}
function getBadgeStyles(color: BadColor) {
  switch (color) {
    case "#43b581":
  ...
  return(
    <div className="w-full flex  items-start justify-start">
      <div className="flex items-center mb-2">
        <Image src={avatarSrc} alt={avatarAlt} width={40} height={40} className="object-cover mr-3 rounded-full" />
      </div>
      ...
      <div className="w-full max-w-xl">
       ...
       <div className="bg-[#2f3136] text-sm w-full rounded mb-4 mt-1.5 p-3">
        ...
        {Object.entries(content).map(([key, value]) => (
         <p 
          key={key}
          className="text-[#dcddde] text-sm/6"
         >
          <span className="text-[#b9bbe]">{key}:</span> {value}
         </p>
        ))}
